...
# ==========================================
# 0. STREAMING CHECKPOINT SYSTEM
# Initializes a local CSV log to record Optuna trials in real-time.
# This prevents data loss during long GPU stacking processes.
# ==========================================

LOG_CSV = "V39_Checkpoint_win.csv"
...
def engineer_features(df):
    # V34 Baseline Feature Engineering
    df = df.copy()
    # 1. Flag infants (Age <= 4)
    df['Is_Infant'] = (df['Age'] <= 4).astype(int)
    # 2. Extract Group and ID from PassengerId, calculate GroupSize
    df[['GROUP', 'ID']] = df['PassengerId'].str.split('_', expand=True)
    df['GroupSize'] = df['GROUP'].map(df['GROUP'].value_counts().to_dict())
    # 3. Split Cabin into Deck, Num, and Side; discretize Cabin Num into 5 quantiles
    df[['Deck', 'Num', 'Side']] = df['Cabin'].str.split('/', expand=True)
    df['Num'] = pd.to_numeric(df['Num'])
    df['CabinRegion'] = pd.qcut(df['Num'], q=5, labels=['Q1', 'Q2', 'Q3', 'Q4', 'Q5'])
    # 4. Zero out spending for CryoSleep passengers and children (Age <= 12)
    ...
    # 5. Calculate TotalSpend and apply log1p transformation to handle skewness
...
if __name__ == '__main__':
    print("=== V39: CATBOOST + XGBOOST OPTUNA STACKING (Multi-Rank Win Edition) ===")
    print("Hardware Target: Windows | CPU (CatBoost) + RTX 4060 (XGBoost)\n")
    
    # Load raw data and extract target PassengerIds for final submission
    ...
    # Handle missing values: Fill categorical NaNs with 'Unknown' string
    # Impute numerical features using median strategy to handle outliers robustly
    ...
    # ==========================================
    # PHASE 1: THE ANCHOR - CATBOOST (STATIC BASELINE)
    # Train CatBoost using the pre-optimized V34 parameters.
    # This serves as the stable base probability matrix for our stacking.
    # Uses CPU multi-threading to free up GPU VRAM for XGBoost later.
    # ==========================================
    ...
    # ==========================================
    # PHASE 2: OPTUNA STACKING OPTIMIZATION
    # Dynamically search for the best XGBoost hyperparameters alongside
    # the optimal fusion weight and decision threshold using GPU acceleration.
    # ==========================================
    def objective(trial):
        # Define search space for XGBoost hyper-parameters (GPU Accelerated)
        xgb_params = {
        ...
        # Define search space for dynamic blending parameters
        # 'weight' controls the XGBoost ratio vs CatBoost (Anchor)
        # 'threshold' optimizes the final boolean classification boundary
        weight = trial.suggest_float('weight', 0.0, 1.0)
        threshold = trial.suggest_float('threshold', 0.35, 0.65)
        
        xgb_oof_proba = np.zeros(len(Y))
        
        # 5-Fold Cross Validation loop for XGBoost OOF generation
        for train_idx, val_idx in skf.split(XY_TRAIN, Y):
            X_tr, X_va = XY_TRAIN[train_idx], XY_TRAIN[val_idx]
            y_tr, y_va = Y[train_idx], Y[val_idx]
        ...
        # The Fusion Process (Soft Voting):
        # Blend XGBoost and CatBoost OOF probabilities based on the current trial's weight.
        # Apply the dynamic threshold to convert probabilities to binary predictions.    
    ...
    # Retrieve all completed trials from the SQLite database and sort by CV score (descending)
    ...
    with open('V39_parameter_win.txt', 'w', encoding='utf-8') as f:
        f.write("=== V39 Multi-Rank Output Parameters ===\n\n")
        f.write(f"Global Best CV: {study.best_value:.5f}\n\n")
        
        # Extract specific metrics and parameters for the current target rank
        for rank in target_ranks:
            if rank <= len(trials_df):
                ...
                # Phase 3 Retraining:
                # Train the optimal XGBoost model for this rank on the ENTIRE training dataset
                # to maximize predictive power for the hidden test set.
                xgb_model = xgb.XGBClassifier(**xgb_params)
                xgb_model.fit(XY_TRAIN, Y)
                xgb_test_proba = xgb_model.predict_proba(XY_TEST)[:, 1]
                
                # Apply the specific rank's fusion weight and threshold to the test set predictions
                final_test_proba = (weight * xgb_test_proba) + ((1.0 - weight) * cb_test_proba)
                final_preds = (final_test_proba >= threshold).astype(bool)
                
                filename = f'V39_submission_rank{rank}_win.csv'
